...
** Last Version:         
** Descriptions:        朗方MDB刷卡器驱动程序相关函数声明                     
**------------------------------------------------------------------------------------------------------
** Created by:          gzz
...
uint8_t LFReaderDevInit()
{
	...
	if(ComStatus == 1)
	{
		...
		*/
		...
		//LFReaderDevProcess(&Bvalue,0,0,&ReaderType);//poll,可以通过这一条指令再得到上一条的数据回复	
		LFReaderUpdateConfigData(ReaderRdBuff);//3返回setup的信息，比如level,比例因子等
	}
	...
	VMCValue[3] = 0xff;
	VMCValue[4] = 0xff;
	VMCValue[5] = 0x00;//Minimum Price – scaled
	VMCValue[6] = 0x00; 
	VMCValue[7] = 0x00; 
	VMCValue[8] = 0x00; 
	VMCValue[9] = 0x11;
...
{	
	...
	Request[1] = 0x00;
	Request[2] = 0x00;
	Request[3] = HUINT16(ItemPrice);//Item Price - scaled交易出货单价
	Request[4] = LUINT16(ItemPrice);
	Request[5] = 0x00;;//Item Number交易出货数量
	Request[6] = 0x01;
		...
		{	
			*readerStatus = ReaderRdBuff[i];
			switch(ReaderRdBuff[i])
			{
				...
				case RTV_READER_CONFIG_INFO:
					TraceReader("\r\n2DrvReaderSetup");
					LFReaderUpdateConfigData(ReaderRdBuff);//2返回setup的信息，比如level,比例因子等
					return;
