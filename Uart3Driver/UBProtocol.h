...
#ifndef __UBPROTOCOL_H__
#define __UBPROTOCOL_H__
...
struct VP_Mission
{
	...
	//向PC机下发设备状态
	uint8_t check_st;                   //"出货检测"设备：0=正常，1=被软件禁用，2=故障，3=不支持出货检测功能
...
struct GoodsMatrix
{
	uint8_t   GoodsType;    //商品编号
	uint16_t    Price;        //商品单价	
};

struct GoodsAttribute
{
	uint8_t   Cabinet;      //箱柜编号
	uint8_t   hd_no;        //货道编号  
	uint8_t   hd_id;        //货道商品编号
	uint8_t   hd_count;     //货道数量
	uint16_t  hd_Price;        //货道单价		
};//单独商品信息
